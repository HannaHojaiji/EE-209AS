package com.example.samebodyauth.esenselib;
...
public class ESenseManager {
    ...
    public ESenseManager(String deviceName, Context context) {
        ...
        Log.d(TAG, "Created ESenseManager");
    }
     ...
     * The event {@link ESenseEventListener#onSensorConfigRead(io.esense.esenselib.ESenseConfig)} is fired when the configuration has been read from the connected device.
     * @return <code>true</code> if the request was successfully made,
     *         <code>false</code> otherwise
     */
     ...
     * The event {@link ESenseSensorListener#onSensorChanged(io.esense.esenselib.ESenseEvent)} is fired every time a new sample is available from the connected device.
    ...
    /**
     * Get bluetooth device object form the eSenseManager
     */
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            for (BluetoothGattService s : gatt.getServices()) {
                for (BluetoothGattCharacteristic c : s.getCharacteristics()) {
                    Log.d("DBG-BASE-MANAGER", " Services Discovered " + c.getUuid().toString());
                    //Log.d("DBG-BASE-MANAGER", " Service Values: " + c.getValue().toString());
